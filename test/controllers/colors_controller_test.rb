...
class ColorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create color" do
    assert_difference("Color.count") do
      post colors_url, params: { color: { alpha: @color.alpha, color: @color.color, fill_id: @color.fill_id, stop: @color.stop } }
    end
  ...
  test "should update color" do
    patch color_url(@color), params: { color: { alpha: @color.alpha, color: @color.color, fill_id: @color.fill_id, stop: @color.stop } }
